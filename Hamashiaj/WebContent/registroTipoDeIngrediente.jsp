...
<title> Registro de Tipo de Ingredientes</title>
</head>
<body>
<h1><b>Registro de Tipo de Ingredientes</b></h1>
<c:if test="${tipoingrediente != null}">
                           <%--  <form action="${pageContext.request.contextPath}/ActualizarEmpleado?accion=edita&codigo=${empleado.codigo}"  method="post"> --%>
                           <form action="TipoDeIngredienteServlet?action=actualizar&id=${tipoingrediente.id}"  method="POST">
                        </c:if>
                        <c:if test="${tipoingrediente == null}">
                           <%--  <form action="${pageContext.request.contextPath}/AgregarEmpleado?accion=agregar&codigo=${empleado.codigo}"  method="POST"> --%>
                            <form action="TipoDeIngredienteServlet?action=registrar"  method="POST">
                        </c:if>

  <div class="form-group row">
    <label for="inputEmail3" class="col-sm-2 col-form-label">Descripcion:</label>
    <div class="col-sm-10">
      <input type="text" name="descripcion" value="${tipoingrediente.descripcion}"  class="form-control">
    </div>
  </div>
